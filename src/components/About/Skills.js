        ...
        <section className='w-full flex flex-col p-20 border-b-2 border-solid border-dark text-dark'>
            ...
            <ul className='flex flex-wrap mt-8 justify-start'>
            {
                SkillList.map((skill, index) => {
                    return (
                        <li key={index} className='font-semibold inline-block capitalize text-2xl py-5 px-12 border-2
                        border-dark rounded mr-6 mb-6 hover:scale-105 transition-all ease duration-200 cursor-pointer'>
                            {skill}
                        </li>
                    ) 
                })
